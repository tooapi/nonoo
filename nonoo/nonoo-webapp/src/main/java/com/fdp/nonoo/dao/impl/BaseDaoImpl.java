...
public abstract class BaseDaoImpl<T, ID extends Serializable> implements
	...
	protected Long queryCount(CriteriaQuery<T> query, List<Filter> filters) {
		Assert.notNull(query);
		Assert.notNull(query.getSelection());
		Assert.notEmpty(query.getRoots());
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
	...
	private Root<T> getRoot(CriteriaQuery<?> query, Class<T> paramClass) {
		if ((query != null) && (query.getRoots() != null)&& (paramClass != null)) {
			Iterator iterator = query.getRoots().iterator();
			while (iterator.hasNext()) {
	...
	private void join(From<?, ?> from, From<?, ?> to) {
		...
		while (iterator.hasNext()) {
			...
			Join join2 = to.join(join1.getAttribute().getName(),join1.getJoinType());
			...
			if ((filter.getOperator() == Filter.Operator.eq)&& (filter.getValue() != null)) {
				...
				if ((filter.getIgnoreCase() != null)
					...
					predicate = criteriaBuilder.and(predicate, criteriaBuilder
							.notEqual(criteriaBuilder.lower(root.get(filter
									.getProperty())), ((String) filter
				...
				predicate = criteriaBuilder.and(predicate, criteriaBuilder.gt(
						root.get(filter.getProperty()),
				...
				predicate = criteriaBuilder.and(predicate, criteriaBuilder.lt(
						root.get(filter.getProperty()),
						(Number) filter.getValue()));
				...
				predicate = criteriaBuilder.and(predicate, criteriaBuilder.ge(
						root.get(filter.getProperty()),
						(Number) filter.getValue()));
				...
				predicate = criteriaBuilder.and(predicate, criteriaBuilder.le(
						root.get(filter.getProperty()),
						(Number) filter.getValue()));
				...
				predicate = criteriaBuilder.and(
						predicate,
						root.get(filter.getProperty()).in(
								new Object[] { filter.getValue() }));
	...
	private void getPageFilter(CriteriaQuery<T> query, Pageable pageable) {
		...
		if (root == null)
			return;
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		Predicate predicate = (query.getRestriction() != null) ? query
		...
		if ((StringUtils.isNotEmpty(pageable.getSearchProperty()))
			...
			predicate = criteriaBuilder.and(predicate, criteriaBuilder.like(
					root.get(pageable.getSearchProperty()),
